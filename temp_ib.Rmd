...
ggplot(clim_mwy, aes(x = year, y = tmax)) +
  geom_point(aes(color = "Maximum Temperature")) +
  stat_smooth(method="lm", col="red") +
  scale_y_continuous(limits=c(min(clim_mwy$tmin), max(clim_mwy$tmax)))+
  geom_point(data=clim_mwy, aes(x = year, tmin, color = "Minimum Temperature")) +
  stat_smooth(data=clim_mwy, aes(x = year, tmin), col="blue", method="lm") +
  labs(y = "Average Annual Temperature (Farenheit)",
       x = "Year",
       colour = "") +
  scale_color_manual(values = colors) +
  annotate(geom="text", x=2010, y=52, label="+0.07 F/year",
              color="red") +
  annotate(geom="text", x=2010, y=35, label="+0.05 F/year",
              color="blue") +
  theme_minimal()

ggsave("temp_ave.jpeg")
