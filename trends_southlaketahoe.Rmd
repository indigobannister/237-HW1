...
The value of the slope is 0.013 F/year, and is statistically significant.
...
Now we find a positive trend: -0.025F/year, or 0.71F/decade. But notice that there seem to be some outliers toward the end of the record...
...
# early portion
res_early = lm(tmin~dt, data=subset(clim.mwy, clim.mwy$dt %in% c(1965:1980)))
summary(res_early)
confint(res_early,"dt", level=0.90)
ggplot(subset(clim.mwy, clim.mwy$dt %in% c(1965:1980)), aes(x=dt, y=tmin)) + stat_summary(fun.y="mean", geom="point", col="red", size=4)+theme(axis.text=element_text(size=14, face="bold"), axis.title=element_text(size=14, face="bold")) + geom_smooth(method="lm")
...
t.test(subset(clim.mwy$tmin, clim.mwy$dt %in% 1965:1990), subset(clim.mwy$tmin, clim.mwy$dt %in% 1995:2004))
